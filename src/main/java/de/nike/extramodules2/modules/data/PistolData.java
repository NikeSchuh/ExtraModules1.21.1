...
public class PistolData implements ModuleData<PistolData> {
    ...
    public void addInformation(Map<Component, Component> map, @Nullable ModuleContext context, boolean stack) {
        if(criticalChance > 0)  map.put(Component.translatable("module.extramodules2.pistol.crit"), TranslationUtils.string( format.format(criticalChance * 100) + "%"));
        if(criticalDamage > 0)  map.put(Component.translatable("module.extramodules2.pistol.critdmg"), TranslationUtils.string( format.format(criticalDamage * 100) + "%"));
        if(extraBaseDamage > 0) map.put(Component.translatable("module.extramodules2.pistol.basedmg"), TranslationUtils.string("+"+ format.format(extraBaseDamage)));
        if(fireTicks > 0 ) map.put(Component.translatable("module.extramodules2.pistol.firetime"), TranslationUtils.string( format.format(fireTicks / 20F) + "s"));
    }
